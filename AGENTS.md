...
Act as an experienced software engineer and technical writer.
...
Use well spaced ASCII graphs for schemas.
...
Write with verbs and nouns. Use minimal adjectives except when describing products or concepts.
...
Use ptimary/secondary instead of master/slave.
...
### Deployment Process
1. Consul: Deploy service discovery cluster
2. Nomad: Deploy workload orchestrator
3. Vault: Deploy secret management cluster (uses Consul as storage backend)
4. Mimir Loki Tempo deployed via Nomad job specifications
5. Alloy: Deploy observability stack

### Configuration Notes
- Services configured via systemd units in `/etc/systemd/system/`
- Binaries installed to `/opt/bin/`
- Configurations stored in `/etc/nomad.d/`, `/etc/vault.d/` and similar paths
